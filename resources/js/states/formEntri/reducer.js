...
function fentriReducer(dt = [], action = {}) {
    switch (action.type) {
      case actType.__form:  
        // return {
        //   dform : action.payload.dform.map(v=>{ return {...v, data:(v.data.length>20?JSON.parse(atob(v.data)):[])}}),
        //   Kategori:action.payload.Kategori.map(v=>{ return {...v, data:(v.data.length>20?JSON.parse(atob(v.data)):[])}})
        // };
